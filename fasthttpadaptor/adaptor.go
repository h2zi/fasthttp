...
// recycles the writer once the server is done with it. Close fails further
// reads before the pooled buffer is recycled; a discarded body may still be
// read by a compression goroutine.
...
func (b *doneBody) WriteTo(dst io.Writer) (int64, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	if b.closed {
		return 0, io.ErrClosedPipe
	}
	return b.r.WriteTo(dst)
}
